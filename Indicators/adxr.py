...
# Function to calculate ADXR
def calculate_adxr(df, period=14):
    # Ensure high, low, and close columns are present
    if not all(col in df.columns for col in ['high', 'low', 'close']):
        raise ValueError("Missing required columns in DataFrame")

    # Calculate the ADXR using TA-Lib
    adxr = ta.ADXR(df['high'], df['low'], df['close'], timeperiod=period)

    # Combine with timestamp and close price in the output
    result = [
        [idx, row['timestamp'], {'close': row['close'], 'adxr': adxr_value}]
        for idx, (row, adxr_value) in enumerate(zip(df.to_dict('records'), adxr))
    ]
...
# Calculate ADXR
adxr_result = calculate_adxr(ohlcv_data)
adxr_result2 = calculate_adxr(ohlcv_data2)

# Print the results
print("1 MINUTE CHART")
for entry in adxr_result:
    print(entry)
print("")
print("5 MINUTE CHART")
for entry in adxr_result2:
    print(entry)