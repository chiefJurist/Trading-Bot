...
def calculate_ma(df, period=14):
    ...
    # Calculate MA (Moving Average) using TA-Lib
    ma_values = ta.MA(df['close'], timeperiod=period)
    
    # Combine with timestamp and close price in the output
    result = [
        [idx, row['timestamp'], {'close': row['close'], 'ma': ma}]
        for idx, (row, ma) in enumerate(zip(df.to_dict('records'), ma_values))
    ]
...
ma_result = calculate_ma(ohlcv_data)
ma_result2 = calculate_ma(ohlcv_data2)

# Print the result
print("1 MINUTE CHART - MA (Moving Average)")
for entry in ma_result:
    print(entry)
print("")
print("5 MINUTES CHART - MA (Moving Average)")
for entry in ma_result2:
    print(entry)