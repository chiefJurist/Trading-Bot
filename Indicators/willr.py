...
# Function to calculate WILLR (Williams %R)
def calculate_willr(df, period=14):
    ...
    willr_values = ta.WILLR(df['high'], df['low'], df['close'], timeperiod=period)

    # Combine with timestamp, close price, and Williams %R values in the output
    result = [
        [idx, row['timestamp'], {'close': row['close'], 'willr': willr}]
        for idx, (row, willr) in enumerate(zip(df.to_dict('records'), willr_values))
    ]
...
willr_result = calculate_willr(ohlcv_data)
willr_result2 = calculate_willr(ohlcv_data2)

# Print the result
print("1 MINUTE CHART - WILLR")
for entry in willr_result:
    print(entry)
print("")
print("5 MINUTES CHART - WILLR")
for entry in willr_result2:
    print(entry)